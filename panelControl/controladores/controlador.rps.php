<script>
            ...
            success: function(response) {
                if(response != "0") {
                    ...
                    for(var i = 0; i < usuarios.length; i++)
                        $("#rps").append('<div class="card cardNegra cardRp mb-3" style="max-width: 540px;"><div class="row no-gutters"><div class="col-md-4"><img src="' + usuarios[i].foto + '" class="card-img" alt="profile-pic"></div><div class="col-md-5"><div class="card-body"><h5 class="card-title tituloConfCard">' + usuarios[i].nombre + '</h5><p class="card-text numCardConf2">' + usuarios[i].total + " / " + usuarios[i].personas + '</p></div></div><div class="col-md-3 znBtns"><div class="editar"> <h5 class="dorado text-center"><a class="linkity" href="?page=8&rp='+ usuarios[i].idUser +'">Más Información</a></h5> </div></div></div></div>');
    ...
    function changeTipoStaff(me) {
        $("#rps").empty();
        $("#rps").html('<div class="loader">Loading...</div>');
        if(me.value == "0"){
            cargarRps();
        }
        else {
